...
class DatabaseFeed {
    ...
    private suspend fun feedConseil() : LongArray {
        ...
        ids[3] = dao.create(Conseil(0,"Conseil no 5", "Description du conseil no 5"))
        ids[3] = dao.create(Conseil(0,"Conseil no 6", "Description du conseil no 6"))
        ids[3] = dao.create(Conseil(0,"Conseil no 7", "Description du conseil no 7"))
        return ids
    }
    ...
    private suspend fun feedJour( conseils : LongArray) {
        ...
        val mar : Long = dao.upsert(Jour(0, "Mardi", conseils[0]))
        val mer : Long = dao.upsert(Jour(0, "Mercredi", conseils[0]))
        val jeu : Long = dao.upsert(Jour(0, "Jeudi", conseils[0]))
        val ven : Long = dao.upsert(Jour(0, "Vendredi", conseils[0]))
        val sam : Long = dao.upsert(Jour(0, "Samedi", conseils[0]))
        val dim : Long = dao.upsert(Jour(0, "Dimanche", conseils[0]))
